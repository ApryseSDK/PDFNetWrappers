...
func TestTransPDFHebrew(t *testing.T) {
    ...
    TransPDFExtractXLIFF(doc, outputPath + "translation-test-hebrew.xlf", options)

    // Save the fielded PDF
    doc.Save(outputPath + "translation-test-hebrew-fielded.pdf", uint(SDFDocE_linearized))
    ...
    TransPDFApplyXLIFF(doc, inputPath + "translation-test-(en_to_he).xlf", options)

    // Save the translated PDF
    doc.Save(outputPath + "translation-test-hebrew.pdf", uint(SDFDocE_linearized))
    doc.Close()
