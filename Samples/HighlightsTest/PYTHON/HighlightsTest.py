...
def main():
    ...
    search_text = "Federal"
    char_ranges = []
    ofs = page_text.find(search_text)
    while ofs != -1:
        cr = CharRange()
        cr.index = ofs
        cr.length = len(search_text)
    ...
    while hlts.HasNext():
        # In Python bindings, quads are typically returned directly
        ...
        for i in range(quad_count):
            ...
            highlight = HighlightAnnot.Create( doc.GetSDFDoc(), Rect(x1,y1,x2,y2) )
